...
for i in range(0,len(polyline_groups)):
    ...
    print(block.getNodeCoor(1))

    bodies.addAvatar(block)



down = pre.rigidJonc(model=mod, material=earth, center=numpy.array([3., -0.25]),color='GREEN', axe1=3.50, axe2=0.25)
bodies.addAvatar(down)

down.imposeDrivenDof(component=[ 2, 3], dofty='vlocy')
down.imposeDrivenDof(component=[1],dofty='vlocy',description='evolution',evolutionFile='evolution.dat')
